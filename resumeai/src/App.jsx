...
function Pill({ text, color = GOLD, bg = "rgba(201,168,80,.08)", border = "rgba(201,168,80,.2)" }) {
  return <span style={{ fontSize: 9.5, background: bg, border: `1px solid ${border}`, borderRadius: 3, padding: "2px 8px", color, letterSpacing: .3, whiteSpace: "nowrap" }}>{text}</span>;
}
...
const arrowBtn = { fontSize: 9, color: GOLD, background: "none", border: "1px solid rgba(201,168,80,.25)", borderRadius: 2, padding: "0 5px", cursor: "pointer", lineHeight: "15px" };
...
function ResumeSections({ resume, upd, order, move }) {
  return order.map((sec, idx) => {
    const props = { resume, upd, canUp: idx > 0, canDown: idx < order.length - 1, onUp: () => move(idx, -1), onDown: () => move(idx, 1) };
    if (sec === "summary" && resume.summary)
      return (
        <div key="summary" style={{ marginBottom: 22 }}>
          <SectionLabel title="Professional Summary" {...props} />
          <Field value={resume.summary} onChange={v => upd(c => c.summary = v)} multiline style={{ fontSize: 13.5, color: "#2a2a2a", lineHeight: 1.8 }} />
        </div>
      );
    ...
    if (sec === "skills" && resume.skills?.length)
      return (
        <div key="skills" style={{ marginBottom: 22 }}>
          ...
          <Field value={resume.skills.join(", ")} onChange={v => upd(c => c.skills = v.split(",").map(s => s.trim()).filter(Boolean))} multiline style={{ fontSize: 12.5, color: "#2a2a2a", lineHeight: 1.8 }} placeholder="Skills separated by commas" />
        </div>
      );
...
export default function App() {
  ...
  const MSGS = ["Analyzing job requirements…","Matching ATS keywords…","Rewriting bullet points…","Crafting your summary…","Calculating match score…"];
  ...
  useEffect(() => {
    if (!loading) return;
    const iv = setInterval(() => setLoadingMsgIdx(i => (i + 1) % MSGS.length), 2400);
    return () => clearInterval(iv);
  }, [loading]);

  // localStorage-based storage for Netlify (no window.storage)
  const lsGet = (k) => { try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : null; } catch { return null; } };
  const lsSet = (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} };
  const lsDel = (k) => { try { localStorage.removeItem(k); } catch {} };

  const loadVersions = () => {
    const ids = lsGet("v-ids") || [];
    setVersions(ids.map(id => lsGet(`v:${id}`)).filter(Boolean));
  };
  const saveVersion = (r) => {
    const id = uid();
    lsSet(`v:${id}`, { id, label: `${r.title || "Resume"} · ${new Date().toLocaleDateString()}`, resume: r, jd, atsScore: r.atsScore, saved: Date.now() });
    const ids = lsGet("v-ids") || [];
    lsSet("v-ids", [...ids.slice(-9), id]);
    loadVersions();
  };
  const deleteVersion = (id) => {
    lsDel(`v:${id}`);
    lsSet("v-ids", (lsGet("v-ids") || []).filter(i => i !== id));
    loadVersions();
  };
  const loadBookmarks = () => setBookmarks(lsGet("bookmarks") || []);
  const toggleBookmark = (job) => {
    const bms = lsGet("bookmarks") || [];
    const next = bms.find(b => b.id === job.id) ? bms.filter(b => b.id !== job.id) : [...bms, job];
    lsSet("bookmarks", next);
    setBookmarks(next);
  };
  ...
  const isL = t => loading && loadingType === t;

  const buildContent = async () => {
    if (inputMode === "file" && file?.type === "application/pdf") {
      const b64 = await f2b(file);
      return [{ type: "document", source: { type: "base64", media_type: "application/pdf", data: b64 } }, { type: "text", text: `Job Description:\n\n${jd}\n\nReturn only the JSON.` }];
    }
  ...
  const handleOptimize = async () => {
    ...
    try {
      const r = await claude(RESUME_SYS(tone), await buildContent(), 4500);
      setResume(r); setScoreOnly(null); setActiveTab("resume");
      silentParseJD(); saveVersion(r);
  ...
  const handleScoreOnly = async () => {
    ...
    try { const sd = await claude(SCORE_SYS, await buildContent(), 1000); setScoreOnly(sd); setResume(null); }
  ...
  const handleCoverLetter = async () => {
    ...
    try { const cl = await claude(COVER_SYS, `Resume JSON:\n${JSON.stringify(resume)}\n\nJob Description:\n${jd}`, 2000); setCoverLetter(cl); setActiveTab("cover"); }
  ...
  const handleParseJD = async () => {
    ...
    try { const p = await claude(JD_SYS, jd, 900); setParsedJD(p); setShowJDPanel(true); }
  ...
  const handleJobSearch = async () => {
    ...
    try {
      const ctx = resume ? `Candidate: ${resume.title}, Skills: ${resume.skills?.slice(0,6).join(", ")}.` : "";
      const results = await claude(JOB_SYS, `Search: "${jobQuery}". ${ctx}`, 3200);
      setJobResults(Array.isArray(results) ? results : []);
    } catch(e) { console.error(e); setErr("Job search failed."); }
    finally { setLoading(false); }
  };

  const handleReOptimize = async (newJD) => {
    if (!resume) return;
    setJd(newJD); setActiveTab("resume"); setErr("");
    setLoading(true); setLoadingType("optimize"); setLoadingMsgIdx(0);
    try {
      const r = await claude(RESUME_SYS(tone), `Existing resume JSON:\n${JSON.stringify(resume)}\n\nNew Job Description:\n${newJD}\n\nRe-optimize for this new role. Return only the JSON.`, 4500);
      setResume(r); setScoreOnly(null); saveVersion(r);
    ...
    <div style={{ height: "100vh", background: BG, display: "flex", flexDirection: "column", fontFamily: "Outfit,sans-serif", color: T1, overflow: "hidden" }}>
      ...
      <div style={{ height: 50, borderBottom: `1px solid rgba(201,168,80,.1)`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", flexShrink: 0, background: "rgba(0,0,0,.35)" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {resume && <span style={{ fontSize: 8.5, color: T2, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 4, padding: "4px 10px" }}>✎ click any field to edit</span>}
          {resume && <button onClick={handlePrint} style={{ fontSize: 9.5, color: GOLD, background: "none", border: `1px solid rgba(201,168,80,.25)`, borderRadius: 4, padding: "5px 12px", cursor: "pointer", fontFamily: "Outfit,sans-serif" }}>↓ Export PDF</button>}
          {(resume || scoreOnly || coverLetter) && <button onClick={() => { setResume(null); setScoreOnly(null); setCoverLetter(null); setFile(null); setPasteText(""); setJd(""); setParsedJD(null); setShowJDPanel(false); setOriginalText(""); setActiveTab("resume"); }} style={{ fontSize: 9.5, color: T2, background: "none", border: "1px solid rgba(255,255,255,.07)", borderRadius: 4, padding: "5px 12px", cursor: "pointer", fontFamily: "Outfit,sans-serif" }}>↩ Start Over</button>}
        </div>
      </div>
        ...
        <div style={{ width: 345, minWidth: 345, background: SIDE, overflowY: "auto", borderRight: `1px solid ${T3}`, padding: "20px 17px", display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            ...
            {inputMode === "file" ? (
              <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)}
                ...
                {file ? (<><div style={{ fontSize: 24, marginBottom: 5 }}>📄</div><div style={{ fontSize: 11.5, color: GOLD, fontWeight: 500, wordBreak: "break-all" }}>{file.name}</div><div style={{ fontSize: 9, color: T2, marginTop: 3 }}>{(file.size/1024).toFixed(0)} KB · click to change</div></>) : (<><div style={{ fontSize: 28, opacity: .15, marginBottom: 8 }}>↑</div><div style={{ fontSize: 12, color: T1, opacity: .55 }}>Drop your resume here</div><div style={{ fontSize: 9.5, color: T2, marginTop: 3 }}>PDF, DOCX, or TXT · click to browse</div></>)}
              </div>
            ) : (
              <textarea value={pasteText} onChange={e => setPasteText(e.target.value)} placeholder="Paste your resume text here…" rows={8} style={{ width: "100%", background: "rgba(255,255,255,.02)", border: `1px solid ${T3}`, borderRadius: 6, padding: 10, color: T1, fontSize: 11, fontFamily: "Outfit,sans-serif", resize: "vertical", outline: "none", lineHeight: 1.65, boxSizing: "border-box" }} />
            )}
          </div>
          ...
          <div>
            <div style={{ ...sideLabel, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {jd.trim() && <button onClick={handleParseJD} disabled={loading} style={{ fontSize: 8.5, color: GOLD, background: "none", border: "none", cursor: loading ? "not-allowed" : "pointer", fontFamily: "Outfit,sans-serif", padding: 0 }}>{isL("jd") ? "Analyzing…" : "Analyze JD →"}</button>}
            </div>
            <textarea value={jd} onChange={e => setJd(e.target.value)} placeholder="Paste the target job description here…" rows={8} style={{ width: "100%", background: "rgba(255,255,255,.02)", border: `1px solid ${T3}`, borderRadius: 6, padding: 10, color: T1, fontSize: 11, fontFamily: "Outfit,sans-serif", resize: "vertical", outline: "none", lineHeight: 1.65, boxSizing: "border-box" }} />
            {showJDPanel && parsedJD && (
              <div style={{ background: "rgba(201,168,80,.04)", border: `1px solid rgba(201,168,80,.2)`, borderRadius: 7, padding: "12px 14px", marginTop: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ fontSize: 12, color: GOLD, fontWeight: 600 }}>{parsedJD.role}{parsedJD.company ? ` @ ${parsedJD.company}` : ""}</div>
                  <button onClick={() => setShowJDPanel(false)} style={{ fontSize: 13, color: T2, background: "none", border: "none", cursor: "pointer" }}>✕</button>
                </div>
                ...
                {parsedJD.required?.length > 0 && <div style={{ marginBottom: 8 }}><div style={{ fontSize: 8, color: "#f87171", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>Must Have</div>{parsedJD.required.map((r,i) => <div key={i} style={{ fontSize: 10, color: "#f87171", marginBottom: 3, paddingLeft: 8, borderLeft: "2px solid rgba(248,113,113,.3)", lineHeight: 1.45 }}>{r}</div>)}</div>}
                {parsedJD.preferred?.length > 0 && <div style={{ marginBottom: 8 }}><div style={{ fontSize: 8, color: "#fbbf24", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>Nice to Have</div>{parsedJD.preferred.map((r,i) => <div key={i} style={{ fontSize: 10, color: "#fbbf24", marginBottom: 3, paddingLeft: 8, borderLeft: "2px solid rgba(251,191,36,.3)", lineHeight: 1.45 }}>{r}</div>)}</div>}
                {parsedJD.keywords?.length > 0 && <div><div style={{ ...sideLabel, marginBottom: 5 }}>ATS Keywords</div><div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>{parsedJD.keywords.map((k,i) => <Pill key={i} text={k} />)}</div></div>}
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <button onClick={handleOptimize} disabled={loading} style={{ padding: "13px 0", background: isL("optimize") ? "rgba(201,168,80,.18)" : `linear-gradient(135deg,${GOLD},#9c7b22)`, color: isL("optimize") ? T2 : "#000", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 600, letterSpacing: 2.5, textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", fontFamily: "Outfit,sans-serif", transition: "all .2s" }}>
              {isL("optimize") ? MSGS[loadingMsgIdx] : "✦  Optimize Resume"}
            </button>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <button onClick={handleScoreOnly} disabled={loading} style={{ padding: "8px 0", background: "transparent", color: isL("score") ? T2 : T1, border: `1px solid ${T3}`, borderRadius: 5, fontSize: 9.5, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", fontFamily: "Outfit,sans-serif" }}>{isL("score") ? "Scoring…" : "Score Only"}</button>
              <button onClick={handleCoverLetter} disabled={loading || !resume} style={{ padding: "8px 0", background: "transparent", color: (!resume || isL("cover")) ? T2 : T1, border: `1px solid ${T3}`, borderRadius: 5, fontSize: 9.5, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase", cursor: (loading || !resume) ? "not-allowed" : "pointer", fontFamily: "Outfit,sans-serif" }}>{isL("cover") ? "Writing…" : "Cover Letter"}</button>
            </div>
          </div>
          ...
          {scoreOnly && !resume && (
            <div style={{ background: "rgba(255,255,255,.015)", border: `1px solid ${T3}`, borderRadius: 8, padding: "18px 14px", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {scoreOnly.recommendations?.length > 0 && <div><div style={sideLabel}>Recommendations</div>{scoreOnly.recommendations.map((r,i) => <div key={i} style={{ fontSize: 10.5, color: "#9de89a", marginBottom: 5, paddingLeft: 9, borderLeft: "2px solid rgba(74,222,128,.2)", lineHeight: 1.5 }}>{r}</div>)}</div>}
              {scoreOnly.missingKeywords?.length > 0 && <div><div style={sideLabel}>Missing Keywords</div><div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>{scoreOnly.missingKeywords.map((k,i) => <Pill key={i} text={k} color="#f87171" bg="rgba(248,113,113,.07)" border="rgba(248,113,113,.2)" />)}</div></div>}
              {scoreOnly.gaps?.length > 0 && <div><div style={sideLabel}>Gaps Found</div>{scoreOnly.gaps.map((g,i) => <div key={i} style={{ fontSize: 10.5, color: "#fbbf24", marginBottom: 5, paddingLeft: 9, borderLeft: "2px solid rgba(251,191,36,.2)", lineHeight: 1.5 }}>{g}</div>)}</div>}
              <button onClick={handleOptimize} disabled={loading} style={{ padding: "9px 0", background: `linear-gradient(135deg,${GOLD},#9c7b22)`, color: "#000", border: "none", borderRadius: 5, fontSize: 10, fontWeight: 600, letterSpacing: 1.5, textTransform: "uppercase", cursor: "pointer", fontFamily: "Outfit,sans-serif" }}>Now Optimize →</button>
            </div>
          )}
          ...
          {resume && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ background: "rgba(255,255,255,.015)", border: `1px solid ${T3}`, borderRadius: 8, padding: "16px 12px" }}><ArcGauge score={resume.atsScore} /></div>
              {resume.gaps?.length > 0 && <div><div style={sideLabel}>Gaps to Address</div>{resume.gaps.map((g,i) => <div key={i} style={{ fontSize: 10.5, color: "#fbbf24", marginBottom: 5, paddingLeft: 9, borderLeft: "2px solid rgba(251,191,36,.2)", lineHeight: 1.5 }}>{g}</div>)}</div>}
              {resume.improvements?.length > 0 && <div><div style={sideLabel}>Improvements Made</div>{resume.improvements.map((imp,i) => <div key={i} style={{ fontSize: 10.5, color: "#9de89a", marginBottom: 5, paddingLeft: 9, borderLeft: "2px solid rgba(74,222,128,.2)", lineHeight: 1.5 }}>{imp}</div>)}</div>}
              {resume.addedKeywords?.length > 0 && <div><div style={sideLabel}>Keywords Added</div><div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>{resume.addedKeywords.map((k,i) => <Pill key={i} text={k} />)}</div></div>}
              {resume.matchedKeywords?.length > 0 && <div><div style={sideLabel}>Keywords Matched</div><div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>{resume.matchedKeywords.map((k,i) => <Pill key={i} text={k} color="#4ade80" bg="rgba(74,222,128,.07)" border="rgba(74,222,128,.2)" />)}</div></div>}
            </div>
          )}
          ...
          {versions.length > 0 && (
            <div>
              <div style={{ ...sideLabel, display: "flex", justifyContent: "space-between" }}><span>Saved Versions</span><span style={{ color: T2 }}>{versions.length}</span></div>
              {[...versions].reverse().map(ver => (
                <div key={ver.id} style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${T3}`, borderRadius: 5, padding: "8px 10px", marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 10, color: T1, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{ver.label}</div><div style={{ fontSize: 9, color: T2, marginTop: 2 }}>ATS {ver.atsScore || "—"}</div></div>
                  <button onClick={() => { setResume(ver.resume); setJd(ver.jd || ""); setActiveTab("resume"); }} style={{ fontSize: 9, color: GOLD, background: "none", border: "none", cursor: "pointer", fontFamily: "Outfit,sans-serif", padding: 0 }}>Load</button>
                  <button onClick={() => deleteVersion(ver.id)} style={{ fontSize: 9, color: "#e57373", background: "none", border: "none", cursor: "pointer", fontFamily: "Outfit,sans-serif", padding: 0 }}>✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ borderBottom: `1px solid ${T3}`, display: "flex", alignItems: "center", padding: "0 18px", flexShrink: 0, background: "rgba(0,0,0,.25)" }}>
            {[["resume","Resume"],["cover","Cover Letter"],["diff","Before / After"],["jobs","Job Search"]].map(([t, lbl]) => (
              <button key={t} onClick={() => setActiveTab(t)} style={{ padding: "12px 14px", fontSize: 11, fontFamily: "Outfit,sans-serif", cursor: "pointer", border: "none", background: "transparent", color: activeTab === t ? T1 : T2, borderBottom: activeTab === t ? `2px solid ${GOLD}` : "2px solid transparent", marginBottom: -1, transition: "all .15s", letterSpacing: .3, fontWeight: activeTab === t ? 500 : 400 }}>{lbl}</button>
            ))}
            {activeTab === "resume" && resume && (
              <div style={{ marginLeft: "auto", display: "flex", gap: 5, alignItems: "center" }}>
                ...
                {[["classic","Classic"],["modern","Modern"],["minimal","Minimal"]].map(([t, lbl]) => (
                  <button key={t} onClick={() => setTemplate(t)} style={{ padding: "4px 10px", fontSize: 9.5, fontFamily: "Outfit,sans-serif", cursor: "pointer", border: `1px solid ${template === t ? GOLD : "rgba(255,255,255,.07)"}`, borderRadius: 3, background: template === t ? "rgba(201,168,80,.1)" : "transparent", color: template === t ? GOLD : T2, transition: "all .15s" }}>{lbl}</button>
                ))}
              </div>
            )}
          </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", background: "#080810", display: "flex", justifyContent: "center", padding: "36px 28px 72px" }}>
              {!resume ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 16, opacity: .4 }}>
                  <svg width="50" height="62" viewBox="0 0 50 62" fill="none"><rect x="1" y="1" width="48" height="60" rx="3" stroke={GOLD} strokeWidth="1.5"/><line x1="9" y1="18" x2="41" y2="18" stroke={GOLD} strokeWidth="1" strokeOpacity=".6"/><line x1="9" y1="26" x2="41" y2="26" stroke={GOLD} strokeWidth="1" strokeOpacity=".4"/><line x1="9" y1="34" x2="33" y2="34" stroke={GOLD} strokeWidth="1" strokeOpacity=".3"/><line x1="9" y1="46" x2="41" y2="46" stroke={GOLD} strokeWidth="1" strokeOpacity=".18"/><line x1="9" y1="54" x2="37" y2="54" stroke={GOLD} strokeWidth="1" strokeOpacity=".18"/></svg>
                  <div style={{ textAlign: "center", lineHeight: 1.8 }}><div style={{ fontSize: 13, color: T2 }}>Your optimized resume will appear here</div><div style={{ fontSize: 10, color: T2, opacity: .6 }}>Upload resume · Paste job description · Optimize</div></div>
                </div>
              ) : (
            ...
            <div style={{ flex: 1, overflowY: "auto", background: "#080810", display: "flex", justifyContent: "center", padding: "36px 28px 72px" }}>
              {!coverLetter ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 14, opacity: .4 }}>
                  ...
                  <div style={{ textAlign: "center" }}><div style={{ fontSize: 13, color: T2 }}>No cover letter yet</div><div style={{ fontSize: 10, color: T2, opacity: .7, marginTop: 5 }}>Optimize your resume first, then click "Cover Letter"</div></div>
                </div>
              ) : (
                <div style={{ width: "100%", maxWidth: 700, background: "#fff", boxShadow: "0 16px 80px rgba(0,0,0,.75)", padding: "60px 64px", fontFamily: "Cormorant Garamond,Georgia,serif", color: "#1a1a1a" }}>
                  <div style={{ fontSize: 11, color: "#bbb", marginBottom: 34, fontFamily: "Outfit,sans-serif" }}>{new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</div>
                  {[coverLetter.greeting, coverLetter.opening, coverLetter.body, coverLetter.closing].map((para, i) => para && <div key={i} style={{ fontSize: 14, lineHeight: 1.9, color: i === 0 ? "#1a1a1a" : "#2a2a2a", marginBottom: i === 0 ? 28 : 20, whiteSpace: "pre-wrap" }}>{para}</div>)}
          ...
          {activeTab === "diff" && (
            <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
              <div style={{ flex: 1, borderRight: `1px solid ${T3}`, padding: "24px", overflowY: "auto" }}>
                ...
                {originalText ? <pre style={{ fontFamily: "Outfit,sans-serif", fontSize: 11, color: T1, lineHeight: 1.75, whiteSpace: "pre-wrap", opacity: .65, margin: 0 }}>{originalText}</pre> : <div style={{ fontSize: 11, color: T2, opacity: .6, lineHeight: 1.7 }}>{file ? `PDF uploaded: ${file.name}\n\nOriginal text not extractable in diff view.\nThe PDF was sent directly to Claude for analysis.` : "Use 'Paste Text' mode to see the before/after comparison."}</div>}
              </div>
              <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
                ...
                {resume ? (
                  <div style={{ fontFamily: "Outfit,sans-serif", fontSize: 11, color: T1, lineHeight: 1.8 }}>
                    ...
                    {resume.summary && <div style={{ marginBottom: 18, opacity: .85 }}><span style={{ color: GOLD, fontWeight: 600, fontSize: 8.5, letterSpacing: 2, textTransform: "uppercase" }}>Summary — </span>{resume.summary}</div>}
                    {resume.experience?.map((exp, i) => (<div key={i} style={{ marginBottom: 15 }}><div style={{ fontWeight: 600 }}>{exp.role} @ {exp.company}</div><div style={{ opacity: .5, fontSize: 10, marginBottom: 5 }}>{exp.duration} · {exp.location}</div>{exp.bullets.map((b, bi) => <div key={bi} style={{ paddingLeft: 14, marginBottom: 4, opacity: .8 }}>• {b}</div>)}</div>))}
                    {resume.skills?.length > 0 && <div style={{ marginTop: 8 }}><span style={{ color: GOLD, fontWeight: 600, fontSize: 8.5, letterSpacing: 2, textTransform: "uppercase" }}>Skills — </span>{resume.skills.join(" · ")}</div>}
                  </div>
          ...
          {activeTab === "jobs" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                ...
                <button onClick={handleJobSearch} disabled={loading} style={{ padding: "10px 20px", background: isL("jobs") ? "rgba(201,168,80,.2)" : GOLD, color: isL("jobs") ? T2 : "#000", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", fontFamily: "Outfit,sans-serif" }}>{isL("jobs") ? "Searching…" : "Search"}</button>
              </div>
              {bookmarks.length > 0 && !selectedJob && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ ...sideLabel, marginBottom: 9 }}>Saved Jobs ({bookmarks.length})</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                    {bookmarks.map(job => <div key={job.id} onClick={() => setSelectedJob(job)} style={{ background: "rgba(201,168,80,.06)", border: `1px solid rgba(201,168,80,.2)`, borderRadius: 5, padding: "5px 12px", fontSize: 11, color: GOLD, cursor: "pointer" }}>{job.title} @ {job.company}</div>)}
                  </div>
                </div>
              )}
              {selectedJob ? (
                  ...
                  <div style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${T3}`, borderRadius: 10, padding: "26px 24px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 20 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                        <button onClick={() => toggleBookmark(selectedJob)} style={{ fontSize: 10, color: bookmarks.find(b => b.id === selectedJob.id) ? GOLD : T2, background: "none", border: `1px solid ${T3}`, borderRadius: 4, padding: "6px 12px", cursor: "pointer", fontFamily: "Outfit,sans-serif" }}>{bookmarks.find(b => b.id === selectedJob.id) ? "★ Saved" : "☆ Save"}</button>
                        <button onClick={() => { resume ? handleReOptimize(selectedJob.description) : (setJd(selectedJob.description), setActiveTab("resume")); }} style={{ fontSize: 10, color: "#000", background: GOLD, border: "none", borderRadius: 4, padding: "6px 16px", cursor: "pointer", fontFamily: "Outfit,sans-serif", fontWeight: 600 }}>{resume ? "Re-optimize →" : "Use This JD →"}</button>
                      </div>
                    </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {jobResults.length === 0 && !loading && <div style={{ textAlign: "center", padding: "70px 0", opacity: .35 }}><div style={{ fontSize: 40, marginBottom: 14 }}>🔍</div><div style={{ fontSize: 13, color: T2 }}>Search for jobs above</div><div style={{ fontSize: 10, color: T2, marginTop: 6, opacity: .7 }}>Claude generates relevant listings tailored to your query and profile</div></div>}
                  {jobResults.map(job => (
                    <div key={job.id} onClick={() => setSelectedJob(job)} style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${T3}`, borderRadius: 8, padding: "16px 18px", cursor: "pointer", transition: "border-color .15s" }}
                      onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(201,168,80,.3)"} onMouseLeave={e => e.currentTarget.style.borderColor = T3}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); toggleBookmark(job); }} style={{ fontSize: 16, color: bookmarks.find(b => b.id === job.id) ? GOLD : T2, background: "none", border: "none", cursor: "pointer", padding: "0 4px", marginLeft: 12 }}>{bookmarks.find(b => b.id === job.id) ? "★" : "☆"}</button>
                      </div>
                    </div>
                  ))}
                </div>
